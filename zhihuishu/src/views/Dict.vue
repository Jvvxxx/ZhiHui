  ...
  <div id="ShuSe">
    <el-row class="mDd">
      <el-col :span="6" :offset="2">
        <template>
          字典名称
          <el-input class="niq" v-model="value" placeholder="请输入内容"></el-input>
        </template>
      </el-col>
      <el-col :span="6" :offset="1">
        <template>
          字典类型
          <el-input class="niq" v-model="value2" placeholder="请输入内容"></el-input>
        </template>
      </el-col>
...
<template>
  ...
  <el-radio v-model="radio" label="2">停用</el-radio>
    ...
    <!-- 修改 -->
    <el-dialog title="修改字典信息" :visible.sync="dialogVisible1" width="width: 41%">
      <el-form
        :model="numberValidateForm"
        ref="numberValidateForm"
        label-width="100px"
        class="demo-ruleForm"
      >
        <el-form-item label="字典名称" prop="age" v-model="bclass">
          <el-input
            type="text"
        ...
        <el-form-item label="字典类型" prop="age" v-model="bclass">
          <el-input
            type="text"
  ...
  <el-radio v-model="updateForm.status" label="2">停用</el-radio>
        ...
        <el-form-item label="备注" prop="age" v-model="bclass">
          <el-input
            type="text"
...
export default {
                ...
                "i-button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.axios({
                        url:
                          "http://122.112.253.28:8095/prod-api/sys/dict/" +
                          that.userID,
                        method: "GET",
                        headers: {
  ...
  created() {
    ...
    this.axios({
      method: "get",
      url: url + "/sys/dict/list?pageNum=1&pageSize=10&beginTime=&endTime=",
      headers: {
        Authorization: window.sessionStorage.token
      }
    }).then(replace => {
  ...
  methods: {
    change(pageIndex) {
      ...
      this.axios({
        ...
        url:
          url +
          `/sys/dict/list?pageNum=${pageIndex}&pageSize=${that.pageSize}`,
        headers: {
          Authorization: window.sessionStorage.token
        }
      }).then(replace => {
    ...
    delClass(id) {
      this.$confirm(
        ...
        .then(() => {
          ...
          this.axios({
            method: "DELETE",
            url: url + "/sys/dict/deleteByIds/" + id,
            headers: {
              Authorization: window.sessionStorage.token
            }
          }).then(delDate => {
    ...
    select() {
      ...
      this.axios({
        method: "get",
        url:
          url +`/sys/dict/list?pageNum=1&pageSize=10&name=${that.value}&type=${that.value2}&status=${that.ztai}&beginTime=&endTime=`,
        headers: {
          Authorization: window.sessionStorage.token
        }
      }).then(relter => {
    ...
    Cha() {
      ...
      this.axios({
        method: "POST",
        url: url + `/sys/dict/create`,
        headers: {
          Authorization: window.sessionStorage.token
        },
        data: {
          name: that.numberValidateForm.age3,
          type: that.numberValidateForm.age,
          remark: that.numberValidateForm.age2,
          status: "1",
        }
      }).then(addDate => {
        console.log(addDate);
    ...
    updata() {
      ...
      this.axios({
        method: "PUT",
        url: url + "/sys/dict/update/" + that.userID,
        headers: {
          Authorization: window.sessionStorage.token
        },

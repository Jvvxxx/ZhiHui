  ...
  <div>
    <el-row class="mDd">
      <el-col :span="8" :offset="1">
        <template>
          卫生扣分项
          <el-input class="niq" v-model="input1" placeholder="请输入卫生扣分项"></el-input>
        </template>
      </el-col>
    ...
    <!-- 添加 -->
    <el-dialog title="添加卫生扣分信息" :visible.sync="dialogVisible" width="width: 41%">
      <el-form
      ...
      >
        <el-form-item
          ...
          v-model="bclass"
          :rules="[
      { required: true, message: '卫生扣分项不能为空'},
    ]"
        >
          <el-input
            type="text"
        ...
        <el-form-item
          ...
          v-model="bclass"
          :rules="[
      { required: true, message: '卫生扣分值内容不能为空'},
    ]"
        >
          <el-input
            type="text"
            placeholder="请输入卫生扣分值"
    ...
    <!-- 修改 -->
    <el-dialog title="修改卫生扣分信息" :visible.sync="dialogVisible1" width="width: 41%">
      <el-form
        :model="numberValidateForm"
        ref="numberValidateForm"
        label-width="120px"
        class="demo-ruleForm"
      >
        <el-form-item label="短信模板标题" prop="age" v-model="updateForm.deductOption">
          <el-input
            type="text"
            placeholder="请输入短信模板标题名称"
            disabled
            v-model="updateForm.deductOption"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="短信模板内容" prop="age" v-model="updateForm.deductValue">
          <el-input
            type="text"
            placeholder="请输入班级名称"
            v-model="updateForm.deductValue"
            autocomplete="off"
          ></el-input>
        </el-form-item>
      </el-form>
...
export default {
                ...
                "i-button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.axios({
                        url:
                          "http://122.112.253.28:8095/prod-api/sysset/hygienededuct/" +
                          that.userID,
                        method: "GET",
                        headers: {
  ...
  mounted() {
    ...
    this.axios({
      method: "get",
      url: url + "/sysset/hygienededuct/list?pageNum=1&pageSize=10",
      headers: {
        Authorization: window.sessionStorage.token
      }
    }).then(replace => {
    ...
    this.axios({
      method: "get",
      url: url + "/sys/dict/detail/dictType/sms_type",
      headers: {
        Authorization: window.sessionStorage.token
      }
    }).then(classth => {
  ...
  methods: {
    change(pageIndex) {
      ...
      this.axios({
        ...
        url:
          url +
          `/sysset/hygienededuct/list?pageNum=${pageIndex}&pageSize=${that.pageSize}`,
        headers: {
          Authorization: window.sessionStorage.token
        }
      }).then(replace => {
    ...
    select() {
      ...
      this.axios({
        method: "get",
        url:
          url +`/sysset/hygienededuct/list?pageNum=1&pageSize=10&deductOption=${that.input1}&status=${that.value2}`,
        headers: {
          Authorization: window.sessionStorage.token
        }
      }).then(selectDate => {
    ...
    delClass(id) {
      this.$confirm(
        ...
        .then(() => {
          ...
          this.axios({
            method: "DELETE",
            url: url + "/sysset/hygienededuct/deleteByIds/" + id,
            headers: {
              Authorization: window.sessionStorage.token
            }
          }).then(delDate => {
    ...
    Cha() {
      ...
      this.axios({
        method: "POST",
        url: url + `/sysset/hygienededuct/create`,
        headers: {
          Authorization: window.sessionStorage.token
        },
        data: {
          deductOption: that.numberValidateForm.age,
          deductValue: that.numberValidateForm.age3,

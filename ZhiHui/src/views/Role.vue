...
export default {
  ...
  created() {
    ...
    this.axios({
      method: "get",
      url: url + "/sys/role/list?pageNum=1&pageSize=10&beginTime=&endTime=",
      headers: {
        Authorization: window.sessionStorage.token
      }
    }).then(replace => {
  ...
  methods: {
    ...
    select() {
      ...
      this.axios({
        method: "get",
        url:
          url +`/sys/role/list?pageNum=1&pageSize=10&name=${that.value}&roleKey=${that.value2}&status=${that.ztai}&beginTime=${that.value1}&endTime=${that.value1}`,
        headers: {
          Authorization: window.sessionStorage.token
        }
      }).then(relter => {
    ...
    updata() {
      ...
      this.axios({
        ...
        if (deldata.data.code == 200) {
          ...
          this.axios({
            method: "get",
            url: url + "/sys/role/list?pageNum=1&pageSize=10",
            headers: {
              Authorization: window.sessionStorage.token
            }
          }).then(replace => {
